...
function updateIdCardData (req, res, next) {
  ...
  const checkPhotos = photos => {
    if (typeof photos !== 'object')
      return Promise.reject(httpError("Not an object", 400))

    const isGoodEntry = ([k, photo]) => typeof k === 'string' && typeof photo === 'string'
    if (!_.every(isGoodEntry, Object.entries(photos)))
      return Promise.reject(httpError("Bad form", 400))

    return Promise.resolve(Object.values(photos))
  }

  Promise.all([
    checkPhotos(req.body),
    customers.getById(customerId),
  ])
    .then(([photos, customer]) => {
      if (!customer) { throw httpError('Not Found', 404) }
      return customers.updateIdCardData(photos, customerId)
        .then(() => customer)
    })
