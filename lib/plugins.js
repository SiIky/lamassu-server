...
function plugins (settings, deviceId) {
  function buildCoinRates (cryptoCode, rateRec, withCommission = true) {
    if (!rateRec) return
  ...
  function internalBuildRates (tickers, withCommission = true) {
    const cryptoCodes = configManager.getLocale(deviceId, settings.config).cryptoCurrencies
    const rates = {}
    cryptoCodes.forEach((cryptoCode, i) => rates[cryptoCode] = buildCoinRates(cryptoCode, tickers[i], withCommission))
    return rates
  }
  ...
  function mapCoinSettings (coinParams) {
    console.debug('plugins.js:mapCoinSettings():coinParams:', coinParams)
    const [cryptoNetwork, cryptoCode] = coinParams
    const commissions = configManager.getCommissions(cryptoCode, deviceId, settings.config)
  ...
  function pollQueries () {
    ...
    return Promise.all([
      ...
      .then(([
        ...
        console.debug('lib/plugins.js:pollQueries():testNets:', testNets)
        const coinsWithoutRate = _(testNets)
          .zip(cryptoCodes)
          .map(mapCoinSettings)
          .value()

        const coins = _(batchableCoins)
          .map(it => ({ batchable: it }))
          .zipWith(
            _.assign,
  ...
  function coinRatesAndBalance (fiatCode, cryptoCode) {
    const commissions = configManager.getCommissions(cryptoCode, deviceId, settings.config)
    return Promise.all([
    ...
    .then(([rates, balanceRec]) => {
      ...
      const commissionRates = buildCoinRates(cryptoCode, rates, true)
      ...
      return {
        ...
        /* NOTE: Doesn't seem to be used outside of this file. */
        timestamp: balanceRec.timestamp,

        ask: rates.rates.ask,
        bid: rates.rates.bid,

        cashIn: commissionRates.cashIn,
        cashOut: commissionRates.cashOut
      }
    })
  }

  function fiatBalance (fiatCode, cryptoCode) {
    return coinRatesAndBalance(fiatCode, cryptoCode)
      .then(_.pick(['timestamp', 'balance']))
  }
