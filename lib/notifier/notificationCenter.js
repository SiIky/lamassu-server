...
const fiatBalancesNotify = (fiatWarnings) => {
  return clearOldFiatNotifications(fiatWarnings).then(notInvalidated => {
    return fiatWarnings.forEach(balance => {
      if (_.find(o => {
        const { deviceId, code, cassette, recycler } = o.detail
        return deviceId === balance.deviceId
          && (
            code ?
              code === balance.code
                && (
                  code === LOW_CASH_OUT ? cassette === balance.cassette :
                  code === LOW_RECYCLER_STACKER ? recycler === balance.recycler :
                  code === CASH_BOX_FULL
                ) :
              /* "old" notifs don't have `code`; assume `cassette` notif */
      ...
      const message = balance.code === LOW_CASH_OUT ?
        ...
        `Recycler ${balance.recycler} low or empty!` :
        balance.code === CASH_BOX_FULL ?
        `Cash box full or almost full!` :
        `Cash box full or almost full!` /* Shouldn't happen */
      const detailB = utils.buildDetail(_.pick(['deviceId', 'code', 'cassette', 'recycler'], balance))
