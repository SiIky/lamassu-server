...
function updatePhotos (photos, id, dir) {
  return Promise.resolve(photos)
    .then(photos => {
      const newPatch = {}
      if (_.isEmpty(photos)) {
        logger.debug("lib/customers.js:updatePhotos(): No photos to save")
        return newPatch
      }
      ...
      // create the directory tree if needed
      _.attempt(() => makeDir.sync(dirname))
      logger.debug("lib/customers.js:updatePhotos(): Saving photos")
      const promises = photos.map((imageData, index) => {
        // decode the base64 string to binary data
        const decodedImageData = Buffer.from(imageData, 'base64')
...
/**
 * @param {String} id customer id
 * @param {Object} photos customer latest id card photos
 * @returns {Promise<Object>} new photos to be applied
 */
function updateIdCardData (photos, id) {
  /* TODO: fetch operator id */
  const operatorId = 'id-operator'
  const directory = `${OPERATOR_DATA_DIR}/${operatorId}/${id}/`
  console.log("lib/customers.js:updateIdCardData():directory:", directory)
  photos.forEach(photo => console.log("lib/customers.js:updateIdCardData():photo:", photo))
  return Promise.resolve(photos)
    .then(photos => {
      return updatePhotos(photos, id, directory)
