...
function sendCoins (account, tx, settings, operatorId, feeMultiplier) {
  ...
  return SEND_QUEUE.add(() =>
    generateTx(toAddress, defaultWallet(account), cryptoAtoms, false, cryptoCode)
...
function generateTx (toAddress, wallet, amount, includesFee, cryptoCode, txId) {
  ...
  let data = null
  if (isErc20Token) {
    const contractAddress = coins.utils.getErc20Token(cryptoCode).contractAddress
    const contract = new web3.eth.Contract(ABI.ERC20, contractAddress)
    const contractData = contract.methods.transfer(toAddress, hex(amount))
    data = contractData.encodeABI()
    toAddress = contractAddress
  }

  let value = isErc20Token ? hex(BN(0)) : amount.toString()
  const txTemplate = {
    from: fromAddress,
    to: toAddress,
    value,
  ...
  const promises = [
    pify(isErc20Token ? contractData.estimateGas : web3.eth.estimateGas)(txTemplate),
    isErc20Token ? 0 : pify(web3.eth.getGasPrice)(),
  ...
  return Promise.all(promises)
    ...
    .then(([gas, gasPrice, txCount, baseFeePerGas]) => {
      ...
      const neededPriority = isErc20Token ? null : new BN(web3.utils.toWei('2.0', 'gwei'))
      const maxFeePerGas = isErc20Token ?
        new BN(2).times(baseFeePerGas).plus(maxPriorityFeePerGas) :
        baseFeePerGas.plus(neededPriority)
      const newGasPrice = isErc20Token ? BN(0) : BN.minimum(maxFeePerGas, baseFeePerGas.plus(maxPriorityFeePerGas))

      const toSend = includesFee
        ? new BN(amount).minus(newGasPrice.times(gas))
        : amount
      value = hex(isErc20Token ? BN(0) : toSend)

      if (isErc20Token && includesFee && (toSend.isNegative() || toSend.isZero())) {
