...
const signTransaction = require('./signing')
...
const sendCoins = (account, tx, settings, operatorId) => {
  ...
  const walletPassphrase = account[`${cryptoCode}WalletPassphrase`]
  return checkCryptoCode(cryptoCode)
    .then(() => makeBitGo(account, cryptoCode))
    .then(bitgo => PromiseObject({
      bitgo,
      network: cryptoNetwork(account, cryptoCode, settings, operatorId),
      unsignedTx: bitgo.buildTransaction(),
      keys: bitgo.listKeys(),
    }))
    .then(({ bitgo, network, unsignedTx, keys }) => PromiseObject({
      bitgo,
      txHex: signTransaction(cryptoCode, network, unsignedTx, keys, walletPassphrase),
    }))
    .then(({ bitgo, txHex }) => bitgo.send(txHex))
    .catch(err => {
