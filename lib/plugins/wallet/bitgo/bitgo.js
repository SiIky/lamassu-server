...
const sendCoins = (account, tx, settings, operatorId) => {
  ...
  // TODO(siiky): Get the `walletPassphrase`
  const walletPassphrase = undefined
  return checkCryptoCode(cryptoCode)
    ...
    .then(({ bitgo, network, unsignedTx, keys }) => PromiseObject({
      bitgo,
      signedTx: signing.sign(cryptoCode, network, unsignedTx, keys, walletPassphrase),
    }))
    .then(({ bitgo, signedTx }) => bitgo.sendHalfSignedTransaction(signedTx))
    .catch(err => {
